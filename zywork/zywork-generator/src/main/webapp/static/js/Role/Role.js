...
function loadTable() {
    ...
    $('#data-list').bootstrapTable({
        url: contextPath + '/role/pager-cond',
        dataType: 'json',
        method: 'post',
...
let fieldTitles = {'id':'编号','title':'标题','description':'描述','createTime-date':'创建时间','updateTime-date':'更新时间','isActive':'是否激活'};

window.operateEvents = {
    'click .to-detail': function (e, value, row, index) {
        showRemoteDetailModal('detail-modal', '/role/detail-modal', row, fieldTitles);
    },
    'click .to-edit': function (e, value, row, index) {
        showRemoteEditModal('edit-modal', '/role/edit-modal', 'edit-form', row, validateFields());
    },
    'click .to-inactive': function (e, value, row, index) {
        active('/role/active', row.id, 1, 'data-list', '/role/pager-cond');
    },
    'click .to-active': function (e, value, row, index) {
        active('/role/active', row.id, 0, 'data-list', '/role/pager-cond');
    },
    'click .to-remove': function (e, value, row, index) {
        remove('/role/remove/' + row.id, 'data-list', '/role/pager-cond');
    }
};
