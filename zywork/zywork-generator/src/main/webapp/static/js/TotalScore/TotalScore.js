...
let fieldTitles = {'id':'编号','userId':'用户编号','total':'兑换数额','score':'兑换积分','createTime-date':'创建时间','updateTime-date':'更新时间'};

window.operateEvents = {
    'click .to-detail': function (e, value, row, index) {
        showRemoteDetailModal('detail-modal', '/total-score/detail-modal', row, fieldTitles);
    },
    'click .to-edit': function (e, value, row, index) {
        showRemoteEditModal('edit-modal', '/total-score/edit-modal', 'edit-form', row, validateFields());
    },
    'click .to-inactive': function (e, value, row, index) {
        active('/total-score/active', row.id, 1, 'data-list', '/total-score/pager-cond');
    },
    'click .to-active': function (e, value, row, index) {
        active('/total-score/active', row.id, 0, 'data-list', '/total-score/pager-cond');
    },
    'click .to-remove': function (e, value, row, index) {
        remove('/total-score/remove/' + row.id, 'data-list', '/total-score/pager-cond');
    }
};
