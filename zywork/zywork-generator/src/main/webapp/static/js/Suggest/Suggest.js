...
function loadTable() {
    ...
    $('#data-list').bootstrapTable({
        url: contextPath + '/suggest/pager-cond',
        dataType: 'json',
        method: 'post',
...
let fieldTitles = {'id':'编号','userId':'用户编号','suggest':'投诉建议','createTime-date':'创建时间','updateTime-date':'更新时间'};

window.operateEvents = {
    'click .to-detail': function (e, value, row, index) {
        showRemoteDetailModal('detail-modal', '/suggest/detail-modal', row, fieldTitles);
    },
    'click .to-edit': function (e, value, row, index) {
        showRemoteEditModal('edit-modal', '/suggest/edit-modal', 'edit-form', row, validateFields());
    },
    'click .to-inactive': function (e, value, row, index) {
        active('/suggest/active', row.id, 1, 'data-list', '/suggest/pager-cond');
    },
    'click .to-active': function (e, value, row, index) {
        active('/suggest/active', row.id, 0, 'data-list', '/suggest/pager-cond');
    },
    'click .to-remove': function (e, value, row, index) {
        remove('/suggest/remove/' + row.id, 'data-list', '/suggest/pager-cond');
    }
};
