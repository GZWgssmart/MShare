...
function loadTable() {
    ...
    $('#data-list').bootstrapTable({
        url: contextPath + '/user-bonus/pager-cond',
        dataType: 'json',
        method: 'post',
        contentType: "application/x-www-form-urlencoded",
        idField: 'bonusAccountsId',
        pagination: true,
...
{
	title: '用户编号',
	field: 'bonusAccountsUid',
	align: 'center',
	sortable: true
},
{
	title: '奖励名称',
	field: 'bonusAccountsBonusName',
	align: 'center',
	sortable: true
},
{
	title: '奖励额',
	field: 'bonusAccountsTotal',
	align: 'center',
	sortable: true
},
{
	title: '更新时间',
	field: 'bonusAccountsUpdatedDate',
	align: 'center',
	sortable: true,
...
function formatOperators(value, row, index) {
    ...
    if (row.bonusAccountsIsActive === 0) {
...
let fieldTitles = {'bonusAccountsId':'编号','bonusAccountsUid':'用户编号','bonusAccountsBonusName':'奖励名称','bonusAccountsTotal':'奖励额','bonusAccountsUpdatedDate-date':'更新时间','userId':'编号','userPhone':'手机号','userNickname':'昵称','userHeadicon':'头像地址'};

window.operateEvents = {
    'click .to-detail': function (e, value, row, index) {
        showRemoteDetailModal('detail-modal', '/user-bonus/detail-modal', row, fieldTitles);
    },
    'click .to-edit': function (e, value, row, index) {
        showRemoteEditModal('edit-modal', '/user-bonus/edit-modal', 'edit-form', row, validateFields());
    },
    'click .to-inactive': function (e, value, row, index) {
        active('/user-bonus/active', row.bonusAccountsId, 1, 'data-list', '/user-bonus/pager-cond');
    },
    'click .to-active': function (e, value, row, index) {
        active('/user-bonus/active', row.bonusAccountsId, 0, 'data-list', '/user-bonus/pager-cond');
    },
    'click .to-remove': function (e, value, row, index) {
        remove('/user-bonus/remove/' + row.bonusAccountsId, 'data-list', '/user-bonus/pager-cond');
    }
};
