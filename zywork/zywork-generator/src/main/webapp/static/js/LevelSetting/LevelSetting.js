...
let fieldTitles = {'id':'编号','name':'等级名称','minMoney':'最小金额','maxMoney':'最大金额','transPercent':'转换比例','createTime-date':'创建时间','updateTime-date':'更新时间'};

window.operateEvents = {
    'click .to-detail': function (e, value, row, index) {
        showRemoteDetailModal('detail-modal', '/level-setting/detail-modal', row, fieldTitles);
    },
    'click .to-edit': function (e, value, row, index) {
        showRemoteEditModal('edit-modal', '/level-setting/edit-modal', 'edit-form', row, validateFields());
    },
    'click .to-inactive': function (e, value, row, index) {
        active('/level-setting/active', row.id, 1, 'data-list', '/level-setting/pager-cond');
    },
    'click .to-active': function (e, value, row, index) {
        active('/level-setting/active', row.id, 0, 'data-list', '/level-setting/pager-cond');
    },
    'click .to-remove': function (e, value, row, index) {
        remove('/level-setting/remove/' + row.id, 'data-list', '/level-setting/pager-cond');
    }
};
