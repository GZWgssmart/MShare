...
let fieldTitles = {'id':'编号','bankName':'银行','cardNo':'银行卡号','realName':'真实姓名','phone':'手机号','userId':'用户编号','createTime-date':'创建时间','updateTime-date':'更新时间'};

window.operateEvents = {
    'click .to-detail': function (e, value, row, index) {
        showRemoteDetailModal('detail-modal', '/bank-card/detail-modal', row, fieldTitles);
    },
    'click .to-edit': function (e, value, row, index) {
        showRemoteEditModal('edit-modal', '/bank-card/edit-modal', 'edit-form', row, validateFields());
    },
    'click .to-inactive': function (e, value, row, index) {
        active('/bank-card/active', row.id, 1, 'data-list', '/bank-card/pager-cond');
    },
    'click .to-active': function (e, value, row, index) {
        active('/bank-card/active', row.id, 0, 'data-list', '/bank-card/pager-cond');
    },
    'click .to-remove': function (e, value, row, index) {
        remove('/bank-card/remove/' + row.id, 'data-list', '/bank-card/pager-cond');
    }
};
