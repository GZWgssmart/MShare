...
function loadTable() {
    ...
    $('#data-list').bootstrapTable({
        url: contextPath + '/permission/pager-cond',
        dataType: 'json',
        method: 'post',
...
let fieldTitles = {'id':'编号','title':'标题','permission':'权限字符串','description':'描述','moduleId':'所属模块','createTime-date':'创建时间','updateTime-date':'更新时间','isActive':'是否激活'};

window.operateEvents = {
    'click .to-detail': function (e, value, row, index) {
        showRemoteDetailModal('detail-modal', '/permission/detail-modal', row, fieldTitles);
    },
    'click .to-edit': function (e, value, row, index) {
        showRemoteEditModal('edit-modal', '/permission/edit-modal', 'edit-form', row, validateFields());
    },
    'click .to-inactive': function (e, value, row, index) {
        active('/permission/active', row.id, 1, 'data-list', '/permission/pager-cond');
    },
    'click .to-active': function (e, value, row, index) {
        active('/permission/active', row.id, 0, 'data-list', '/permission/pager-cond');
    },
    'click .to-remove': function (e, value, row, index) {
        remove('/permission/remove/' + row.id, 'data-list', '/permission/pager-cond');
    }
};
